...
export default function AdminStockNewPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    try {
      await adminCreateStock({ ...form, symbol: form.symbol.toUpperCase() });
      ...
      <Shell>
        <div className="max-w-2xl flex flex-col gap-4">
          ...
          <form onSubmit={submit} className="bg-white rounded-xl border p-6 flex flex-col gap-4">
            ...
            <label>Símbolo<input value={form.symbol} onChange={(e) => setForm({ ...form, symbol: e.target.value })} required className="border rounded-lg p-3 w-full" placeholder="MANG3" /></label>
            ...
            <button type="submit" className="bg-[#174f3d] text-white py-3 rounded-lg">Cadastrar ação</button>
          </form>
        </div>
      </Shell>
    </AuthGuard>
