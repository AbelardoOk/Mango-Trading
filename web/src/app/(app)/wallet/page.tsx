      ...
      <Shell>
        <div className="flex flex-col gap-6">
          <div>
            ...
            <p className="text-sm text-[#596b63]">Acompanhe suas posições e patrimônio.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-xl p-6 border"><div className="text-xs text-[#596b63]">Saldo disponível</div><div className="text-xl font-bold">{formatMoney(p.balance)}</div></div>
            <div className="bg-white rounded-xl p-6 border"><div className="text-xs text-[#596b63]">Valor das ações</div><div className="text-xl font-bold">{formatMoney(p.totalCurrentValue)}</div></div>
          </div>
          {isEmpty ? (
            <div className="bg-white rounded-xl border p-12 text-center flex flex-col items-center gap-4">
              ...
              <p className="text-sm text-[#596b63]">Explore o mercado para começar sua carteira virtual.</p>
                ...
                <tbody>
                  {p.items.map((it) => (
                    <tr key={it.id} className="border-t">
                      <td className="p-4"><div className="font-bold">{it.stockSymbol}</div><div className="text-xs text-[#596b63]">{it.stockName}</div></td>
          ...
          <p className="text-xs text-[#596b63]">Patrimônio = saldo + valor atual das ações. Resultado = (preço atual − preço médio) × quantidade.</p>
        </div>
      </Shell>
    </AuthGuard>
  );
